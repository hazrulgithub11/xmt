#!/usr/bin/env python3
"""Sync Payment Received workflows from XMT___Billing_System.ds to local .deluge files."""
...
def extract_payment_received_workflows(content: str) -> dict[str, tuple[str, bool]]:
    ...
    while i < len(lines):
        ...
        block_text = "\n".join(block_lines)
        if "form = Payment_Received" not in block_text:
            i = end + 1
            continue
        if "form = Payment_Received_Line_Item" in block_text:
            i = end + 1
            continue
...
def main() -> None:
    ...
    workflows = extract_payment_received_workflows(content)
