#!/usr/bin/env python3
"""Sync Notify Credits Available workflows from XMT___Billing_System.ds."""
...
OUT_DIR = ROOT / "application" / "forms" / "Notify Credits Available" / "workflow"
ACTIONS_DIR = OUT_DIR / "actions"
WORKFLOW_PREFIX = "\t\t\t"
FORM_MARKER = "form = Notify_Credits_Available"
...
def extract_workflows(content: str) -> dict[str, tuple[str, bool]]:
    ...
    while i < len(lines):
        ...
        block_text = "\n".join(block_lines)
        if FORM_MARKER not in block_text:
            i = end + 1
            continue
