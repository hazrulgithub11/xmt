...
|---|---|---|---|---|
| T1 | Manual create → save → reopen | Credit Notes → Add → pick customer → add line → Save → reopen record | Blueprint = **Draft**; `Credits_Remaining` = `Grand_Total`; no jump to Open/Closed | [ ] |
| T2 | Manual create — actions hidden | On Draft CN, open Credit Notes report actions | **No** Apply Credit / Create Refund / Submit LHDN | [ ] |
| T3 | Convert from invoice | Pick invoice (Sent/Partially Paid/Paid/Overdue) → Convert to Credit Note | CN opens as **Draft**; `Reference_Invoice` = source invoice; CN number assigned **once** on save | [ ] |
| T4 | Convert — no early Open | After convert popup loads, before save | Blueprint stays **Draft** (not Open) | [ ] |
| T5 | Approval loop | Draft → Send for Approval → Approve (pass LHDN taxpayer validation) | **Pending Approval** → **Approved** | [ ] |
| T6 | Reject / resubmit | Reject → edit → Resubmit → Approve | **Rejected** → **Pending Approval** → **Approved** | [ ] |
| T7 | Apply credit — default reference | Approved CN with `Reference_Invoice` (unpaid) → Apply Credits | Source invoice pre-filled in `Credits_To_Apply` | [ ] |
| T8 | Apply credit — balances | Apply partial credit → submit | Invoice `Amount_Due` reduced; CN `Credits_Used` / `Credits_Remaining` correct; stage → **Open** if remaining > 0 | [ ] |
| T9 | Apply credit — full close | Apply full remaining credit | CN stage → **Closed**; `Credits_Remaining` = 0 | [ ] |
| T10 | Refund path | Approved CN → Create Refund Note → save refund | CN `Refund` and `Credits_Remaining` updated; stage Open or Closed per balance | [ ] |
| T11 | Convert + refund only (no apply) | Convert → approve → refund full amount (no apply) | CN **Closed**; LHDN payload still includes source invoice UUID (T13) | [ ] |
| T12 | Subform delete blocked (pre-approval) | Draft CN — Credit Applied / Refund History subforms | Delete row button **hidden** | [ ] |
| T13 | LHDN — button visibility | CN not Closed | Submit LHDN action **hidden** | [ ] |
| T14 | LHDN — submit + payload | Closed CN → Submit Credit Note to LHDN | Submission succeeds; `info reference_invoice_list` includes `Reference_Invoice` UUID when set | [ ] |
