...
| ----- | ---------------------------------- | --------------------------------------------------------- | ------------------------------------------------------ | ------- |
| T09-1 | Manual CN without reference        | Credit Notes → Add → do not pick Reference_Invoice → Save | **Blocked** — "must be linked to a reference invoice"  | [ pass] |
| T09-2 | Manual CN with unsubmitted invoice | Pick invoice with no `Invoice_UUID` → Save                | **Blocked** — "not yet submitted to LHDN"              | [pass ] |
| T09-3 | Manual CN with valid LHDN invoice  | Pick LHDN-validated invoice → Save                        | **Allowed** — Draft created                            | [pass ] |
| T09-4 | Convert from unsubmitted invoice   | Invoice with no UUID → Convert to Credit Note             | **Blocked** with message                               | [ pass] |
| T09-5 | Convert from valid invoice         | LHDN-submitted invoice → Convert                          | **Allowed** — Draft created with Reference_Invoice set | [pass ] |
...
| #     | Scenario                                   | Steps                                        | Expected                                               | Pass    |
| ----- | ------------------------------------------ | -------------------------------------------- | ------------------------------------------------------ | ------- |
| T10-1 | Manual CN — lines load on reference select | Pick Reference_Invoice                       | Line items auto-populate matching source invoice       | [ pass] |
| T10-2 | Manual CN — item code read-only            | Try to change item code on a line            | **Blocked** — field disabled                           | [ pass] |
| T10-3 | Reduce qty below source                    | Lower qty on a line                          | Sub_Total and Grand_Total recalculate correctly        | [ pass] |
| T10-4 | Qty exceeds source                         | Enter qty > source invoice qty               | **Blocked at save** — "exceeds original invoice qty"   | [pass ] |
| T10-5 | Unit price exceeds source                  | Enter unit price > source invoice unit price | **Blocked at save**                                    | [ pass] |
| T10-6 | Change Reference_Invoice in Draft          | Select different reference invoice           | Old lines cleared; new lines from new reference loaded | [ pass] |
